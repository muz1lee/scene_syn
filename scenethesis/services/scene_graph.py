...
<<<<<<< HEAD
    ...
    def _save_mask_and_crop(
        ...
        """
        保存 mask 和 crop 图像到磁盘，供后续 CLIP 检索使用。
        ...
        Returns:
            (mask_path, crop_path): mask 和 crop 的保存路径
        """
        # 生成安全的文件名
        safe_label = label.replace(" ", "_").replace("/", "_")
        timestamp = idx  # 使用索引作为唯一标识
        ...
        try:
            # 保存 mask（如果存在）
            if detection.mask_image:
                mask_filename = f"{safe_label}_{timestamp}_mask.png"
                mask_path = self.mask_dir / mask_filename
                detection.mask_image.save(mask_path)
                print(f"✓ [SceneGraphBuilder] 保存 mask: {mask_path}")

            # 保存 crop
            x1, y1, x2, y2 = detection.bbox
            crop = base_image.crop((max(0, x1), max(0, y1), max(x1 + 1, x2), max(y1 + 1, y2)))
            crop_filename = f"{safe_label}_{timestamp}_crop.png"
            crop_path = self.crop_dir / crop_filename
            crop.save(crop_path)
            print(f"✓ [SceneGraphBuilder] 保存 crop: {crop_path}")
